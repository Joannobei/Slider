import React from "react";

const Section1 = () => {
  return (
    <div className=" bg-[#6772E5] relative  z-50">
      <div className="h-[522px] pt-[84.75px] lg:max-w-[1440px] max-w-[375px] m-auto">
        <div className=" lg:w-[838px] w-[331px] lg:h-[170px] h-[256px] py-[63px] m-auto">
          <h2 className="font-bold lg:text-[46px] text-[24px] text-white text-center">
            Smart reporting tools at your fingertips
          </h2>
          <p className="text-center lg:text-[20px] text-[14px] mt-[32px] text-white">
          Integrate with your favorite accounting software, export your reports, access in-depth analytics and see your new automated financial operating system improve your business process.
          </p>
        </div>
      </div>
    </div>
  );
};

export default Section1;
